 ...
 */
@Service
public class PeopleTempServiceImpl implements PeopleTempService{


    @Autowired
    private PeopleTempMapper peopleTempMapper;
    ...
    @Override
    public PeopleTemp findPeopleTempByName(String name) {
        return peopleTempMapper.findPeopleTempByName(name);
    }

    @Override
    public void findDataGrid(PageInfo pageInfo) {
        pageInfo.setRows(peopleTempMapper.findPeopleTempPageCondition(pageInfo));
        pageInfo.setTotal(peopleTempMapper.findPeopleTempPageCount(pageInfo));
    }

    @Override
    public void addPeopleTemp(PeopleTemp peopleTemp,CommonsMultipartFile file) {

        //当birthday不为空，而是""的时候，需要修改为null，否则插入会有错误
        if (peopleTemp != null){
            if (StringUtils.isEmpty(peopleTemp.getBirthday())){
                peopleTemp.setBirthday(null);
            }
        }
        ...
        peopleTemp.setCode(StringUtilExtra.generateUUID());
    ...
    @Override
    public void updatePeopleTemp(PeopleTemp peopleTemp, CommonsMultipartFile file) {
    ...
    @Override
    public void batchDeletePeopleTempByIds(String[] ids){
        peopleTempMapper.batchDeleteByIds(ids);
    }
    ...
    public boolean insertByImport(CommonsMultipartFile[] files){
        ...
        if(files!=null && files.length>0){
            ...
            List<PeopleTemp> list = new ArrayList<PeopleTemp>();
            ...
            for(int i=0; i<files.length; i++){
                ...
                if( StringUtils.isNotBlank(path)){
                    list=getPeopleTempInfoByExcel(list,path);
                }
            }
            if(list.size()>0){
                flag=peopleTempMapper.insertByImport(list)>0;
            }
        }
        return flag;
    }
     ...
     * @return
     */
    private List<PeopleTemp> getPeopleTempInfoByExcel(List<PeopleTemp> list,String path){
        try {
    ...
    public void exportExcel(HttpServletResponse response,String[] idList){
        List list=peopleTempMapper.selectPeopleTempVoByIds(idList);
        if(list!=null&&list.size()>0){
            XSSFWorkbook workBook;
            OutputStream os;
            String newFileName="临时聘用人员信息.xlsx";
            try {
                workBook = new XSSFWorkbook();
                XSSFSheet sheet= workBook.createSheet("临时聘用人员信息");
                ...
                for(int i=0;i<list.size();i++){
                    ...
                    PeopleTempVo p=(PeopleTempVo)list.get(i);
    ...
    @Override
    public String findPeopleTempIDsByCondition(PageInfo pageInfo) {
        String ids = "";
        pageInfo.setFrom(0);
        pageInfo.setSize(100000);
        List<PeopleTempVo> peopleTempList = peopleTempMapper.findPeopleTempPageCondition(pageInfo);
        if (peopleTempList == null || peopleTempList.size() < 1)
            return ids;


        for(int i=0; i<peopleTempList.size(); i++){
            ids = ids + peopleTempList.get(i).getId().toString() + ",";
        }
