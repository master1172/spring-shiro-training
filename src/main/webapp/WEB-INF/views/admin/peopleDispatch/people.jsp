    ...
    <script type="text/javascript">
        ...
        $(function () {
            dataGrid = $('#dataGrid').datagrid({
                url: '${path}/peopleDispatch/dataGrid',
                fit: true,
                striped: true,
                rownumbers: true,
                pagination: true,
            ...
            parent.$.modalDialog({
                ...
                buttons: [{
                    ...
                    handler: function () {
                        ...
                        //f.submit();
                        if(parent.checkForm()){
                            parent.SYS_SUBMIT_FORM(f,"/peopleDispatch/add",function(data){
                                if(!data["success"]){
                                    parent.$.messager.alert("提示", data["msg"], "warning");
                                }else{
                                    parent.progressClose();
            ...
            parent.$.modalDialog({
                ...
                buttons: [{
                    ...
                    handler: function () {
                        ...
                        //f.submit();
                        if(parent.checkForm()){
                            parent.SYS_SUBMIT_FORM(f,"/peopleDispatch/edit",function(data){
                                if(!data["success"]){
                                    parent.$.messager.alert("提示", data["msg"], "warning");
                                }else{
                                    parent.progressClose();
        ...
        function deleteFun(id) {
            ...
            parent.$.messager.confirm('询问', '您是否要删除当前人员？', function (b) {
                if (b) {
                    progressLoad();
                    $.post('${path}/peopleDispatch/delete',{
                        id: id
                    }, function (result) {
                        if (result.success) {
        ...
        function batchDel(){
            ...
            parent.$.messager.confirm('询问', '您是否要删除所选人员？', function (b) {
                if (b) {
                    progressLoad();
                    $.post('${path}/peopleDispatch/batchDel', {
                        ids: ids.join(",")
                    }, function (result) {
                        if (result.success) {
                            parent.$.messager.alert('提示', result.msg, 'info');
        ...
        function importExcel(){
            parent.$.modalDialog({
                ...
                width: 500,
                height: 300,
                href: '${path}/peopleDispatch/importExcelPage',
                buttons: [{
                    text: '导入',
                    handler: function () {
                        parent.$.modalDialog.openner_dataGrid = dataGrid;//因为添加成功之后，需要刷新这个dataGrid，所以先预定义好
                        ...
                        //f.submit();
                        if(parent.checkForm()){
                            parent.SYS_SUBMIT_FORM(f,"/peopleDispatch/importExcel",function(data){
                                if(!data["success"]){
                                    parent.$.messager.alert("提示", data["msg"], "warning");
                                }else{
                                    parent.progressClose();
        ...
        function exportWord(){
            ...
            if(checkedItems.length==1){
                ...
                form.attr("action",'${path}'+"/peopleDispatch/exportWord");
                $("#downLoadForm").submit();
            }else{
        ...
        function operateFormatter(value,row,index){
            var str = '';
            <shiro:hasPermission name="/peopleDispatch/edit">
            str += $.formatString('<a href="javascript:void(0)" class="user-easyui-linkbutton-edit" data-options="plain:true,iconCls:\'icon-edit\'" onclick="editFun(\'{0}\');" >编辑</a>', row.id);
            </shiro:hasPermission>
            <shiro:hasPermission name="/peopleDispatch/delete">
            str += '&nbsp;&nbsp;|&nbsp;&nbsp;';
