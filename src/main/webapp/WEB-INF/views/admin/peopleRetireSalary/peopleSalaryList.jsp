...
<script type="text/javascript">
    ...
    $(function () {
        salaryGrid = $('#salaryGrid').datagrid({
            url: '${path}/peopleRetireSalary/salaryGrid',
            fit: true,
            striped: true,
        ...
        parent.$.modalDialog({
            ...
            buttons: [{
                ...
                handler: function () {
					...
					if(parent.checkForm()) {
                    parent.SYS_SUBMIT_FORM(f,"/peopleRetireSalary/add",function(data){
                        if(!data["success"]){
        ...
        parent.$.modalDialog({
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    if(parent.checkForm()){
                        parent.SYS_SUBMIT_FORM(f,"/peopleRetireSalary/editSalary",function(data){
                            if(!data["success"]){
                                parent.progressClose();
                                parent.$.messager.alert("提示", data["msg"], "warning");
                            }else{
    ...
    function deleteFun(id) {
        ...
        parent.$.messager.confirm('询问', '您是否要删除当前工资记录？', function (b) {
            if (b) {
                progressLoad();
                $.post('${path}/peopleRetireSalary/delete',{
                    id: id
                }, function (result) {
                    if (result.success) {
        ...
        parent.$.modalDialog({
            ...
            buttons:[{
                ...
                handler: function(){
                    ...
                    if(parent.checkForm()) {
                        parent.SYS_SUBMIT_FORM(f, "/peopleRetireSalary/salaryBaseEdit", function (data) {
                            if (!data["success"]) {
                                parent.progressClose();
                                parent.$.messager.alert("提示", data["msg"], "warning");
                            } else {
